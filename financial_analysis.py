    ...
    try:
        ...
        if rev_raw is not None:
            ...
            if not rev_df.empty and 'type' in rev_df.columns and 'value' in rev_df.columns:
                ...
                rev_df['value'] = pd.to_numeric(rev_df['value'], errors='coerce')
                rev_df = (
        ...
        # 計算 TTM
        # 規則：
        # 1. 若今年已有實際 EPS，就用可得的實際季 + 去年剩餘季組成最近四季
        # 2. 若今年缺最新應有季度，就用營收年增率補那一季
        eps_ttm = None

        this_year_seasons = sorted(
            df[df['year'] == this_year]['season'].dropna().astype(int).unique().tolist()
        )
